...
def calculate_sd_score(video_path, text, pipe, model, gpu_id=0):
    # Load the video
    output_dir = f"../../SDXL_Imgs_gpu{gpu_id}"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dir_prompts", type=str, default='', help="Specify the path of prompts used to generate videos")
    parser.add_argument("--gpu_id", type=int, default=0, help="GPU ID for GPU-specific directories")
    ...
    ###############################
    dir_prompts = args.dir_prompts  ## to use my own prompts
    # dir_prompts =  '../../prompts/'
    ###############################
    ...
    results_dir = f"../../results_gpu{args.gpu_id}"
    os.makedirs(results_dir, exist_ok=True)
    # Set up logging
    log_file_path = f"{results_dir}/{metric}_record.txt"
    ...
    file_handler = logging.FileHandler(filename=f"{results_dir}/{metric}_record.txt")
    ...
    for i in tqdm(range(len(video_paths))):
        ...
        if count == test_num:
            ...
            if metric == 'clip_score':
                ...
                score = calculate_sd_score(video_path, text, pipe, clip_model, args.gpu_id)
