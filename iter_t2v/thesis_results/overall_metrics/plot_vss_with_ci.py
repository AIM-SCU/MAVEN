#!/usr/bin/env python3
"""
Create VSS (Visual Similarity Score) plot with confidence intervals.
...
def compute_vss_with_ci(df, pipeline_name, confidence=0.95):
    """Compute VSS with confidence intervals."""

    col_name = VSS_COLS[pipeline_name]
...
def main():
    """Create VSS plot with confidence intervals."""
    ...
    for pipeline_name, filepath in FILES.items():
        ...
        all_metrics[pipeline_name] = compute_vss_with_ci(df, pipeline_name)
    ...
    ax.set_ylabel('Visual Similarity Score (VSS)', fontsize=18, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(pipelines, fontsize=16)
    ax.tick_params(axis='y', labelsize=16)
    ax.grid(axis='y', alpha=0.3, linestyle='--')
    ax.set_ylim(0, max(means) + max(cis) + 0.15)

    # Tight layout
    plt.tight_layout()

    # Save plots
    pdf_path = OUTPUT_DIR / 'vss_with_ci.pdf'
    png_path = OUTPUT_DIR / 'vss_with_ci.png'
    ...
    csv_path = OUTPUT_DIR / 'vss_with_ci.csv'
    csv_df.to_csv(csv_path, index=False, float_format='%.6f')
    print(f"Saved CSV to: {csv_path}")

    # Print results
    print("\n" + "="*60)
    print("Visual Similarity Score (VSS) Summary")
    print("="*60)
