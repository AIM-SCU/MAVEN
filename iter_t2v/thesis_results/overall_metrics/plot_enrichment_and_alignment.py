#!/usr/bin/env python3
"""
Compute and plot alignment scores for all pipelines.
Each pipeline (base, SA, MAS, MAP) has 2 bars:
1. AS_orig: Alignment with original prompt
2. AS_final: Alignment with refined prompt
"""
...
def compute_metrics_with_ci(df, pipeline_name, confidence=0.95, all_dfs=None):
    """
    Compute AS_orig and AS_final with confidence intervals.

    For agent pipelines:
    AS_orig = AS_{v_agent, Pr_orig}
    AS_final = AS_{v_agent, Pr_final}

    For base pipeline:
    AS_orig = AS_{v_base, Pr_orig}
    AS_final = average of AS_{v_base, Pr_final} across SA, MAS, MAP
    """
...
def main():
    """Create grouped bar plot for alignment scores."""
    ...
    for pipeline_name in FILES.keys():
        ...
        if pipeline_name == 'base':
            ...
            df = all_dfs['SA']
            all_metrics[pipeline_name] = compute_metrics_with_ci(df, pipeline_name, all_dfs=all_dfs)
        else:
            df = all_dfs[pipeline_name]
            all_metrics[pipeline_name] = compute_metrics_with_ci(df, pipeline_name)

    # Prepare data for plotting (left to right order)
    pipelines = ['base', 'SA', 'MAS', 'MAP']
    metric_names = ['AS_orig', 'AS_final']
    ...
    for i, metric in enumerate(metric_names):
        ...
        if metric == 'AS_orig':
            label = r'$\mathrm{AS}(v, \mathrm{Pr}_{\mathrm{orig}})$'
        else:  # AS_final
            label = r'$\mathrm{AS}(v, \mathrm{Pr}_{\mathrm{final}})$'
        ...
        for j, pipeline in enumerate(pipelines):
            ...
            ax.text(x[j] + i * width - width/2, height + ci + 0.005,
                   f'{mean:.3f}',
    ...
    # Save plots
    pdf_path = OUTPUT_DIR / 'enrichment_and_alignment.pdf'
    png_path = OUTPUT_DIR / 'enrichment_and_alignment.png'
    ...
    csv_path = OUTPUT_DIR / 'enrichment_and_alignment.csv'
    csv_df.to_csv(csv_path, index=False, float_format='%.6f')
    print(f"Saved CSV to: {csv_path}")

    # Print results
    print("\n" + "="*80)
    print("Alignment Scores Summary")
    print("="*80)
    for pipeline in pipelines:
        print(f"\n{pipeline}:")
        for metric in metric_names:
            m = all_metrics[pipeline][metric]
            metric_label = "AS(v_agent, Pr_orig)" if metric == "AS_orig" else "AS(v_agent, Pr_final)"
            print(f"  {metric_label:25s}: {m['mean']:.6f} ± {m['ci']:.6f}")
    print("="*80)
