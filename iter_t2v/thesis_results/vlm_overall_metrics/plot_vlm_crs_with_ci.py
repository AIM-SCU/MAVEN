#!/usr/bin/env python3
"""
Create VLM-based CRS plot with confidence intervals and numerical values.
"""
...
def compute_vlm_crs_with_ci(df, pipeline_name, confidence=0.95):
    """Compute VLM CRS metrics with confidence intervals."""

    metrics = {}

    # Column names depend on pipeline
    if pipeline_name == 'base':
        crs_cols = ['vlm_overall_score', 'vlm_person_score', 'vlm_action_score', 'vlm_location_score']
    else:
        crs_cols = ['vlm_v_var_overall_score', 'vlm_v_var_person_score', 'vlm_v_var_action_score', 'vlm_v_var_location_score']

    metric_names = ['VLM_OCRS', 'VLM_PCRS', 'VLM_ACRS', 'VLM_LCRS']
    ...
    # Compute VLM_CRS as average of the four
...
def main():
    """Create VLM CRS plot with confidence intervals."""
    ...
    for pipeline_name, filepath in FILES.items():
        ...
        all_metrics[pipeline_name] = compute_vlm_crs_with_ci(df, pipeline_name)

    # Prepare data for plotting (left to right order)
    pipelines = ['base', 'SA', 'MAS', 'MAP']
    metric_names = ['VLM_CRS', 'VLM_OCRS', 'VLM_PCRS', 'VLM_ACRS', 'VLM_LCRS']
    ...
    ax.set_ylabel('VLM Cultural Relevance Score', fontsize=18, fontweight='bold')
    ...
    # Save plots
    pdf_path = OUTPUT_DIR / 'vlm_crs_with_ci.pdf'
    png_path = OUTPUT_DIR / 'vlm_crs_with_ci.png'
    ...
    csv_path = OUTPUT_DIR / 'vlm_crs_with_ci.csv'
    csv_df.to_csv(csv_path, index=False, float_format='%.6f')
    print(f"Saved CSV to: {csv_path}")

    # Print results
    print("\n" + "="*60)
    print("VLM Cultural Relevance Score (VLM_CRS) Summary")
    print("="*60)
    for pipeline in pipelines:
        print(f"\n{pipeline}:")
        for metric in metric_names:
            m = all_metrics[pipeline][metric]
            print(f"  {metric:15s}: {m['mean']:.4f} ± {m['ci']:.4f}")
    print("="*60)
