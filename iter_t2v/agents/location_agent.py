...
class LocationAgent(AgentBase):
    """
    Refines ONLY the LOCATION portion of a prompt.
    Returns JSON: {"refined_prompt": "...", "justification": "..."}
    """

    def __init__(self, llm_client, name: str = "LocationAgent") -> None:
        super().__init__(name)
    ...
    async def refine(self, prompt: Prompt) -> Prompt:
        persona_line = build_persona_prompt(prompt, slice_="location")

        system_prompt = textwrap.dedent(
            f"""
            {persona_line}

            Your task is to improve only the location part of the prompt.
            Add iconic visual details that make the place recognizable (e.g., landscape, architecture, lighting, environmental elements).
            Do not change the person or the action.

            Return JSON with:
            "refined_prompt": the updated sentence
            "justification": a brief explanation of how your addition makes the location more vivid or culturally recognizable.
            """
        ).strip()
